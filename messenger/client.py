    ...
    def create_message(self):
        ...
        with database_lock:
            if not self.database.check_user(to):
                logger.error(f"Попытка отправить сообщение " f"незарегистрированому получателю: {to}")
                return
        ...
        with database_lock:
            if ask == "in":
                ...
                for message in history_list:
                    print(f"\nСообщение от пользователя: {message[0]} " f"от {message[3]}:\n{message[2]}")
            elif ask == "out":
                history_list = self.database.get_history(from_who=self.account_name)
                for message in history_list:
                    print(f"\nСообщение пользователю: {message[1]} " f"от {message[3]}:\n{message[2]}")
            else:
            ...
            with sock_lock:
                ...
                # Если пакет корретно получен выводим в консоль и записываем в базу.
                    ...
                    if (
                        ...
                        print(f"\n Получено сообщение от пользователя " f"{message[SENDER]}:\n{message[MESSAGE_TEXT]}")
                        ...
                        logger.info(
                            f"Получено сообщение от пользователя " f"{message[SENDER]}:\n{message[MESSAGE_TEXT]}"
                        )
                    else:
...
def main():
    ...
    try:
        ...
        logger.error(f"В ответе сервера отсутствует необходимое поле " f"{missing_error.missing_field}")
        exit(1)
